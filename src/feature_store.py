...
class FeatureStoreByCrypto:
    ...
    def set_RSI(self, window: int = 14):
        ...
        self.data["gain"] = np.where(
            self.data["price_diff"] > 0, self.data["price_diff"], 0
        )
        self.data["loss"] = np.where(
            self.data["price_diff"] < 0, self.data["price_diff"].abs(), 0
        )
        ...
        self.data = self.data.drop(
            columns=["price_diff", "gain", "loss", "AU", "AD", "RS"]
        )

    def set_stochK(self, window: int = 14):
        self.data[f"lowest_stochK_{window}d"] = (
            self.data["close"].rolling(window=window).min()
        )
        self.data[f"highest_stochK_{window}d"] = (
            self.data["close"].rolling(window=window).max()
        )

        self.data["stochK"] = (
            (self.data["close"] - self.data[f"lowest_stochK_{window}d"])
            / (
                self.data[f"highest_stochK_{window}d"]
                - self.data[f"lowest_stochK_{window}d"]
            )
        ...
        self.data = self.data.drop(
            columns=[f"lowest_stochK_{window}d", f"highest_stochK_{window}d"]
        )
    ...
    def set_stochRSI(self, window: int = 14):
        self.data[f"lowest_RSI_{window}d"] = (
            self.data["RSI"].rolling(window=window).min()
        )
        self.data[f"highest_RSI_{window}d"] = (
            self.data["RSI"].rolling(window=window).max()
        )
        self.data["stochRSI"] = (
            (self.data["RSI"] - self.data[f"lowest_RSI_{window}d"])
            / (self.data[f"highest_RSI_{window}d"] - self.data[f"lowest_RSI_{window}d"])
            * 100
        )
        self.data = self.data.drop(
            columns=[f"lowest_RSI_{window}d", f"highest_RSI_{window}d"]
        )
    ...
    def set_CCI(self, window: int = 20):
        ...
        self.data["mean_deviation_TP"] = (
            self.data["TP"]
            .rolling(window=window)
            .apply(lambda x: abs(x - x.mean()).mean(), raw=True)
        )
        self.data["CCI"] = (self.data["TP"] - self.data["MA_TP"]) / (
            0.015 * self.data["mean_deviation_TP"]
        )
    ...
    def set_MACD(
        self, fast_window: int = 12, slow_window: int = 26, for_volume: bool = False
    ):
        if for_volume:
            using_col = "volume"
            result_col = "volMACD"
        ...
        self.data[result_col] = (
            self.data[using_col].rolling(fast_window).mean()
            - self.data["close"].rolling(slow_window).mean()
        )
    ...
    def set_MACD_signal(self, window: int = 9, for_volume: bool = False):
        ...
        self.data[result_col] = (
            self.data[using_col] - self.data[using_col].rolling(window=window).mean()
        )
    ...
    def set_Chaikin(self, fast_window: int = 3, slow_window: int = 10):
        ...
        self.data[f"EMA_AD_{fast_window}d"] = (
            self.data["AD"].rolling(window=fast_window).mean()
        )
        self.data[f"EMA_AD_{slow_window}d"] = (
            self.data["AD"].rolling(window=slow_window).mean()
        )
        self.data["Chaikin"] = (
            self.data[f"EMA_AD_{fast_window}d"] - self.data[f"EMA_AD_{slow_window}d"]
        )

        self.data = self.data.drop(
            columns=["AD", f"EMA_AD_{fast_window}d", f"EMA_AD_{slow_window}d"]
        )
    ...
    def set_markov_regime_switching(self):
        ...
        model = MarkovRegression(
            df["returns"], k_regimes=2, switching_variance=True
        ).fit()
        ...
        self.data = self.data.merge(
            result[["regime"]], left_index=True, right_index=True, how="left"
        )
...
class FeatureStoreByDate:
    ...
    def get_fear_and_greed_indicator(
        ...
        QUALIFY row_number() OVER (PARTITION BY reg_date ORDER BY update_dt DESC) = 1
        """)
        return result
